	...
	try {
		...
		switch(args[0].toLowerCase()) {
			...
			case "logs":
			  ...
			  break;
			case "purchase":
			  var price = 30000; var balance = parseInt(PlaceholderAPI.static.setPlaceholders(Player, "%playerpoints_points%"));
			  ...
			  if(errors != 0) {
			  	...
			  	var Purchase = Java.extend(Runnable, {
			  		run: function() {
			  			var Command = "playerpoints:p take " + Player.getName() + " " + price.toString();
			  			...
			  			Server.dispatchCommand(Console, Command); Server.dispatchCommand(Console, Give);
